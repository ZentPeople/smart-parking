  ...
  async (param) => {
    try {
      ...
      const options = {
        ...
        image:
          "https://lh3.googleusercontent.com/N8LxEaBwVEQ_B31XdQL1_NZ-4QbGK2Jhpvp1i_wJ3HFJASijQtU6BPnGGmSNwF9K_j9lExWOvnT4L96PNH0Vaq4lJM5Qga0_ukTl8g",
...
//which will verify the payment details and redirect to paymentSucces page if successs else redirect to paymentFailure page
export const asynccheckoutRefund = createAsyncThunk(
  "payments/checkoutRefund",
  async (formData) => {
    try {
      ...
      const options = {
        ...
        currency: "INR",
        name: "Smart Parker",
        description: `Payment for refund to ${
          formData.bookerName
        } for booking of a ${formData.vehicleType} at ${
          formData.name
        } between ${dayjs(formData.startTime)} and ${dayjs(formData.endTime)} `,
        image:
          "https://lh3.googleusercontent.com/N8LxEaBwVEQ_B31XdQL1_NZ-4QbGK2Jhpvp1i_wJ3HFJASijQtU6BPnGGmSNwF9K_j9lExWOvnT4L96PNH0Vaq4lJM5Qga0_ukTl8g",
        order_id: order.id, //This is a sample Order ID. Pass the `id` obtained in the response of Step 1
