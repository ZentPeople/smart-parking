  ...
  try {
    ...
    if (!existingUser) {
      return res.status(400).json({
        msg: "No account with this email ID, Create an Account first",
      });
    } else if (existingUser.verified) {
      return res
        .status(200)
        .json({ msg: "You are already verified, you can login directly" });
    }
    ...
    return res
      .status(200)
      .json({ msg: "Vefiy OTP sent to your email To Access Your Account" });
  } catch (err) {
    return res.status(500).json({ msg: "Something went wrong.." });
  }
};
...
exports.verifyEmail = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: "You're Registered Successfully, Login Now" });
  } catch (err) {
    return res.status(500).json({ msg: "Something went wrong.." });
  }
};
  ...
  try {
    ...
    if (!currUser) {
      return res.status(404).json({
        msg: "No user exists with this email, create an account first",
      });
    }
    ...
    const resetCode = jwt.sign(payload, process.env.TOKEN_SECRET, {
      expiresIn: "30m",
    });
    ...
    return res
      .status(200)
      .json({ msg: "Mail sent with link to reset Your password" });
  } catch (err) {
    return res.status(500).json({ msg: "Something went wrong.." });
  }
};
...
exports.resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (decodedData.exp * 1000 < currTimeStamp) {
      return res
        .status(400)
        .json({ msg: "Expired code, send an email again to get a new code" });
    }
    ...
    return res.status(200).json({
      msg: "Password reset successfully, you can login now with new password!",
    });
  } catch (err) {
    return res.status(500).json({ msg: "Something went wrong.." });
  }
